...
export default class Config {
  ...
  async view() {
    ...
    console.log(
      `timezone: ${this.cfg.timezone ||
        `(none configured, using ${moment.tz.guess()})`}`
    );
    console.log('hosts:');
    Object.entries(this.cfg.hosts).forEach(([name, host]) => {
      console.log(`  ${name}:`);
      console.log(`    url   : ${host.url}`);
      console.log(`    key   : ${host.key}`);
      console.log(`    secret: ${host.secret}`);
    });
  }
  ...
  async writeConfig(cfg) {
    ...
    // console.log(`writing to "${cfg._filename}"...\n---\n${formatted}\n---`);
    console.log(`writing to "${cfg._filename}"...`);
  ...
  static timezoneQuestions(zone) {
    ...
    if (zone) {
      ...
      if (filtered.length === 1) {
        ...
        console.log(`unknown timezone "${zone}"`);
      }
    }
