...
export default class Get {
  ...
  async createCommand() {
    ...
    const describe = 'gets orders from a WooCommerce site';
    const builder = yargs => {
      yargs
        .option('status', {
...
Examples:
  ...
  async run(argv, command) {
    ...
    const host = this.hosts[argv.host];
    if (!host) {
    ...
    helpers.out(`retrieved ${wcOrders.length} orders...`);
...
function generatePretty(items, currencies) {
  ...
  const columns = fields.map(f => {
    ...
    if (process.stdout.isTTY) {
      // prettier-ignore
      switch (f.label) {
        case 'order#': config.width = 5;            break;
        case 'date':   config.width = 12;           break;
