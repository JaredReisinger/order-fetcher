...
function simplifyItem(item, order) {
    ...
    simplified.meta = {};
    helpers.dbg(3, 'metadata', item.meta_data);
    for (const m of item.meta) {
        ...
        if (!key) {
            helpers.dbg(0, `order ${order.id} has meta_data with no key: ${JSON.stringify(m)}`);
            continue;
        }
...
function normalizePhone(phone) {
    ...
    switch (phone.length) {
        ...
        case 9:
            ...
            helpers.out(`short phone: ${origPhone}, treating as ${phone}`);
            break;
        default:
            if (phone.length >= 11 && phone.length <= 14) {
                ...
                helpers.out(`long phone: ${origPhone}, treating as ${phone}`);
            } else {
                helpers.out(`unexpected phone: ${origPhone}, leaving as-is`);
                phone = origPhone;
            }
            break;
    }
